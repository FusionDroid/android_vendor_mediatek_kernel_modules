 ...
 *============================================================================
 ****************************************************************************/
#include "mot_cybert_s5kjnsuwmipiraw_Sensor.h"
#include "mot_s5kjns_uw_cali.h"
#define USING_DPHY_N_LANE 4

static void set_group_hold(void *arg, u8 en);
static u16 get_gain2reg(u32 gain);
static int s5kjns_uw_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, s5kjns_uw_set_test_pattern},
#if  ENABLE_S5KJNS_UW_LONG_EXPOSURE
...
#if  ENABLE_S5KJNS_UW_LONG_EXPOSURE
static void s5kjns_uw_set_long_exposure(struct subdrv_ctx *ctx)
{
...
static void s5kjns_uw_set_shutter_frame_length(struct subdrv_ctx *ctx, u64 shutter, u32 frame_length)
{
	...
	/* write shutter */
	s5kjns_uw_set_long_exposure(ctx);
	/* write framelength */
...
static int s5kjns_uw_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	s5kjns_uw_set_shutter_frame_length(ctx, shutter,0);
	return 0;
}
#endif
...
#if ENABLE_S5KJNS_UW_60FPS_PD
static struct SET_PD_BLOCK_INFO_T s5kjns_uw_60fps_pd_info = {
	.i4OffsetX = 0,
	.i4OffsetY = 8,
	.i4PitchX = 8,
	.i4PitchY = 8,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = MOT_CYBERT_S5KJNS_UW_SENSOR_ID,
...
static struct subdrv_ops ops = {
	...
	.close = s5kjns_uw_ops_close,
	.get_frame_desc = common_get_frame_desc,
	.get_csi_param = common_get_csi_param,
	.update_sof_cnt = common_update_sof_cnt,
	.set_ctrl_locker = s5kjns_uw_set_ctrl_locker,
};
...
const struct subdrv_entry mot_cybert_s5kjns_uw_mipi_raw_entry = {
	.name = "mot_cybert_s5kjns_uw_mipi_raw",
	.id = MOT_CYBERT_S5KJNS_UW_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int s5kjns_uw_streaming_control(struct subdrv_ctx *ctx, kal_bool enable)
{
...
static int s5kjns_uw_streaming_off(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	return s5kjns_uw_streaming_control(ctx, KAL_FALSE);
}

static int s5kjns_uw_ops_close(struct subdrv_ctx *ctx)
{
	s5kjns_uw_streaming_control(ctx, KAL_FALSE);
	DRV_LOG_MUST(ctx, "subdrv close \n");
	return ERROR_NONE;
}
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			//sensor id +5 for mot_cybert_s5kjns_uw
			if(*sensor_id == 0x38EE)
			*sensor_id += 5;
...
{
	DRV_LOG(ctx, "E\n");
	DRV_LOG(ctx, "MOT CYBERT S5KJNS_UW init start\n");
	...
	i2c_table_write(ctx, addr_data_pair_init_mot_cybert_s5kjns_uw,
			ARRAY_SIZE(addr_data_pair_init_mot_cybert_s5kjns_uw));
	DRV_LOG(ctx, "MOT CYBERT S5KJNS_UW init end\n");
	DRV_LOG(ctx, "X\n");
}
...
static int s5kjns_uw_set_ctrl_locker(struct subdrv_ctx *ctx,
